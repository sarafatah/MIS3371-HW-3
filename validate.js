...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (ssnInput && ssnError) {
        ssnInput.addEventListener("input", function (e) {
            ...
            ssnError.textContent = (val.length === 11 && !validPattern.test(val)) ? "Invalid SSN format. Use XXX-XX-XXXX." : "";
        });
    ...
    function validatePassword() {
        ...
        const username = getValue("userid").toLowerCase();
        const fname = getValue("firstname").toLowerCase();
        const lname = getValue("lastname").toLowerCase();

        let errors = [];
        if (password.length < 8 || password.length > 30) errors.push("8-30 characters");
        if (!/[A-Z]/.test(password)) errors.push("1 uppercase letter");
        if (!/[a-z]/.test(password)) errors.push("1 lowercase letter");
        if (!/[0-9]/.test(password)) errors.push("1 number");
        if (!/[!@#%^&*()\-_+=\/><.,~]/.test(password)) errors.push("1 special char");
        if (/['"]/.test(password)) errors.push("No quotes allowed");
        if (password.toLowerCase().includes(username)) errors.push("Cannot contain User ID");
        if (password.toLowerCase().includes(fname)) errors.push("Cannot contain first name");
        if (password.toLowerCase().includes(lname)) errors.push("Cannot contain last name");
